...
public class DBMSBoundary {
    ...
    public static ResultSet getConsegneRecenti(){
        ...
        try{
            Connection connection = connectDBMS();
            Statement statement = connection.createStatement();
            resultSet = statement.executeQuery("Select u.nome, p.id_prenotazione, p.id_utente_farmacia, p.data_consegna, p.ricevuta_pdf from vipagepharma_azienda.prenotazione p, vipagepharma_farmacia.utente u where u.id_utente_farmacia=p.id_utente_farmacia and isConsegnato = 1 order by data_consegna desc limit 10"); //vedere se funziona sintassi
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        return resultSet;
    ...
    public static ResultSet getAltreConsegne(){
        ...
        try{
            Connection connection = connectDBMS();
            Statement statement = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            resultSet = statement.executeQuery("Select u.nome, p.id_prenotazione, p.id_utente_farmacia, p.data_consegna, p.ricevuta_pdf from vipagepharma_azienda.prenotazione p, vipagepharma_farmacia.utente u where u.id_utente_farmacia=p.id_utente_farmacia and isConsegnato=1 order by data_consegna  desc limit " + base + ", 10"); //vedere se funziona sintassi
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        return resultSet;
